...
namespace {
	...
	struct test_compute_assert_expression {
		test_function *fun_ptr;
		hyper::model::abortable_computation* ptr; // owned by assert
		hyper::model::compute_assert_expression assert_;
		boost::asio::deadline_timer deadline;
		hyper::model::identifier id;
		int handled_test;
		bool first_time;

		test_compute_assert_expression(boost::asio::io_service& io_s):
			fun_ptr(new test_function()), 
			ptr(new hyper::model::abortable_computation()),
			assert_(io_s, boost::posix_time::milliseconds(20), ptr,
					hyper::logic::function_call("pipo"),
					id, fun_ptr->res(), 0),
			deadline(io_s)
		{
		...
		void handle_first_test(const boost::system::error_code& e)
		{
			if (first_time) {
				...
				assert_.compute(boost::bind(&test_compute_assert_expression::handle_second_test,
										 this, _1));
				deadline.expires_from_now(boost::posix_time::milliseconds(100));
				deadline.async_wait(boost::bind(&test_compute_assert_expression::handle_timeout, 
							this,
		...
		{
			first_time = true;
			assert_.compute(boost::bind(&test_compute_assert_expression::handle_first_test,
									 this, _1));
		}
	};
}

BOOST_AUTO_TEST_CASE ( model_compute_assert_expression_test )
{
	boost::asio::io_service io_s;
	test_compute_assert_expression test(io_s);
	test.handled_test = 0;
	test.async_test();
	io_s.run();
